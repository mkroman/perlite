...
void Client::cmdPing(Network* network, Command* command) {
	network->sendCommand("PONG :%s", command->getParam(0).c_str());
}
...
void Client::cmdNameReply(Network* network, Command* command) {
	...
	UserTable::iterator it2;

	_vector_each(it2, channel->getUsers()) {
		//network->sendCommand("PRIVMSG #uplink :Hello, %s!", (*it2)->getNick().c_str());
	}
}
